...
public class FileSystemUtilsTest {
  ...
  public void testResolveLatest() throws IOException, URISyntaxException {
    ...
    Assert.assertTrue(FileSystemUtils.resolveLatest(resourcePath + "/2018/11/02.dat").endsWith("/root/2018/11/02.dat"));
    Assert.assertTrue(
    ...
    Assert.assertTrue(FileSystemUtils.resolveLatest(resourcePath + "/#LATEST/#LATEST").endsWith("/root/2019/13.dat"));
  }

  private String getPathForResource(String resource) throws URISyntaxException {
    String path = Paths.get(Thread.currentThread().getContextClassLoader().getResource(resource).toURI()).toFile()
        .getAbsolutePath();
    return path;
  }
}
